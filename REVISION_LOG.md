...
### 13. `index.html`, `printing.html`, `equipment.html` — исправление атрибутов изображений
...
### 14. `index.html` — исправление адреса Яндекс Карт
...
### 15. `printing.html` — восстановление стилей под единый стандарт
...
- Добавлен CSS для `.compare-card__gallery` в `css/style.css`
- Расширен `js/main.js`: добавлены материалы `composites` и поля `advantages` для динамического обновления характеристик
- Проверена мобильная адаптивность: карточки складываются в одну колонку, спецификации одноколоночные, галерея 2 колонки

**Затронутые файлы:**
- `printing.html` — обновление структуры блока сравнения и исправление атрибутов изображений
- `css/style.css` — добавлен стиль `.compare-card__gallery`
- `js/main.js` — расширены объекты `plasticData` и `metalData`
